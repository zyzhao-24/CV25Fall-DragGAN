...
with gr.Blocks() as app:
    ...
    with gr.Row():
        ...
        # 左侧 Control Panel
        with gr.Column(scale=3, min_width=300):  # 左侧列保证足够宽度显示 label
            gr.Markdown("### Control Panel")

            # Model & Latent
            with gr.Accordion("Model & Latent", open=True):
            ...
            # Drag 操作
            with gr.Accordion("Drag", open=True):
                with gr.Row():
    ...
    * Note that changing the latent space will reset the image, points and
    ...
    def on_click_start(global_state, image):
        ...
        if len(global_state["points"]) == 0:
            ...
            renderer.last_image = torch.from_numpy(np.array(img_pil)).permute(
            ...
            while True:
                ...
                renderer.last_image = torch.from_numpy(np.array(res_img_pil)).permute(
                    2, 0, 1).float().unsqueeze(0).to(device)
