...
def run_drag_eval(model_pkl, seed=0, out_dir='./eval_out',
    ...
    Image.fromarray(img1).save(os.path.join(out_dir, 'image1_original.png'))
    ...
    if use_cascaded_blending:
        ...
        Image.fromarray(img2).save(os.path.join(out_dir, 'image2_dragged_blended.png'))
    else:
        Image.fromarray(img2).save(os.path.join(out_dir, 'image2_dragged.png'))
    ...
    if use_cascaded_blending:
        vis2.save(os.path.join(out_dir, "image2_dragged_blended_points.png"))
    else:
        vis2.save(os.path.join(out_dir, "image2_points.png"))
    
    # Check if drag actually happened
    print("Step 6.2: Image comparison...")
    img_diff = np.abs(img1.astype(np.float32) - img2.astype(np.float32)).mean()
    print(f"  Mean pixel difference: {img_diff:.4f}")
    if img_diff < 1.0:
        print(f"  WARNING: Images are nearly identical. Drag may not have been applied correctly.")
    ...
    print("  Analyzing individual point movements...")
    for i, (hy, hx) in enumerate(handle_points):
        hy, hx = int(round(hy)), int(round(hx))
        ty, tx = int(round(target_points[i][0])), int(round(target_points[i][1]))
        
        # Extract patch from both images
        patch_size = 15
        y1, y2 = max(0, hy - patch_size), min(img1.shape[0], hy + patch_size)
        x1, x2 = max(0, hx - patch_size), min(img1.shape[1], hx + patch_size)
        
        patch1 = img1[y1:y2, x1:x2]
        patch2 = img2[y1:y2, x1:x2]
        
        if patch1.size > 0 and patch2.size > 0:
            patch_diff = np.mean(np.abs(patch1.astype(np.float32) - patch2.astype(np.float32)))
            target_dist = np.sqrt((ty - hy) ** 2 + (tx - hx) ** 2)
            print(f"    Point {i}: target_dist={target_dist:.1f}px, patch_diff={patch_diff:.2f}")
    
    # Similarity in mask region
    print("  Computing patch-based similarity...")
    similarity = compute_patch_similarity(img1, img2, mask)
    print(f"    Patch similarity (VGG): {similarity:.4f}")
    
    # Matching points distance
    print("  Computing matching points distance...")
    avg_dist, distances = compute_matching_points_distance(
        handle_points, target_points, img1, img2, mask
    )
    print(f"    Avg matching point distance: {avg_dist:.2f} pixels")
    if len(distances) > 0:
        print(f"    Distance range: {min(distances):.2f} - {max(distances):.2f}")
    
    # Pixel-wise difference in mask
    print("  Computing pixel-wise metrics...")
    mask_3d = mask[:, :, np.newaxis]
    img1_f = img1.astype(np.float32) / 255.0
    img2_f = img2.astype(np.float32) / 255.0
    l2_diff = np.sqrt(np.mean(((img1_f - img2_f) ** 2) * mask_3d))
    l1_diff = np.mean(np.abs(img1_f - img2_f) * mask_3d)
    print(f"    L2 diff (masked): {l2_diff:.4f}")
    print(f"    L1 diff (masked): {l1_diff:.4f}")
    ...
    results = {
        ...
        'metrics': {
            'patch_similarity_vgg': float(similarity),
            'matching_points_avg_distance': float(avg_dist),
            'matching_points_distances': [float(d) for d in distances],
            'l2_diff_masked': float(l2_diff),
            'l1_diff_masked': float(l1_diff)
        },
        'output_images': {
