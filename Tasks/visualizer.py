...
from blending import record_mask_snapshot
...
# Fallback: if no pkl files found, try to populate from FreeDrag list or hardcoded default
# This allows the UI to render correctly even if models are missing (user might add them later or paths differ)
if len(valid_checkpoints_dict) == 0:
    print('No pkl files found in cache_dir. Populating with default model list.')
    CKPT_SIZE = {
        'faces.pkl': [512, 0.3, 3],
    ...
    for pkl_name in CKPT_SIZE.keys():
        key = pkl_name.split('.')[0]
        # Assuming they will be in cache_dir even if os.listdir didn't see them (maybe mounted later)
        # or just to satisfy UI.
        valid_checkpoints_dict[key] = osp.join(cache_dir, pkl_name)
        ...
        if len(global_state["points"]) == 0:
            ...
            while True:
                ...
                renderer._render_drag_impl(
                    ...
                    is_drag=True,
                    to_pil=True,
                    tracking_method=global_state['params']['tracking_method'])  # Add tracking_method parameter
