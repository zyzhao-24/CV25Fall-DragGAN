...
#This is core Task2 test version
...
def point_tracking_mix(renderer, feat_resize, points, last_img, curr_img, r3, h, w):
    ...
    with torch.no_grad():
        for j, point in enumerate(points_raft):
            ...
            # L2 最近邻匹配
            ref_feat = renderer.feat_refs[j].reshape(1, -1, 1, 1)
...
def compute_raft_mask_loss(renderer, curr_img, mask_usq, lambda_mask=10):
    ...
    print(f"[raft mask loss] Loss fix computed:{loss_fix}")
    return loss_fix
...
def render_drag_impl(renderer, res,
                     ...
                     reset=False,
                     to_pil=False,
                     tracking_method='L2',  # Add tracking_method parameter
                     fixloss_type='single',
                     use_blended_output=False,
                     **kwargs
                     ):
    G = renderer.G
    ws = renderer.w
    ...
    if is_drag:
        ...
        # print(f"[point tracking]Using tracking method {tracking_method}")
        if tracking_method == 'raft':
            ...
            print(f'No matching point tracking method: {tracking_method}')
        ...
        if mask is not None:
            if mask.min() == 0 and mask.max() == 1:
                ...
                print(f"[loss_motion]Using fix loss type {fixloss_type}")
    ...
    # Check if cascaded blending is needed
        if (hasattr(renderer, 'mask_snapshot_features') and 
            ...
            try:
                ...
                with torch.no_grad():
                    blended_img, blend_features = apply_cascaded_blending(
                        G, ws, 
                        snapshot_features, 
                        blending_mask,
                        label=label,
                ...
                if fixloss_type == 'blended':
                    # Compute loss between blended image and current image in masked region
                    blended_feat_resize = F.interpolate(
                        blend_features[feature_idx].detach(), [h, w], mode='bilinear')
                    blended_loss = F.l1_loss(feat_resize * mask_usq,
                                        blended_feat_resize * mask_usq)
                    loss +=  lambda_mask * blended_loss
                # Use blended image
                if use_blended_output:
                    img = blended_img
