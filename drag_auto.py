...
from core_blending import record_mask_snapshot, apply_cascaded_blending, prepare_blending_mask
...
def to_raft_img(image, device):
    """
    Convert image (PIL / numpy / torch) to RAFT-ready tensor:
    [1,3,H,W], float32, range [0,255], on device
    """
    import numpy as np
    import torch
    from PIL import Image
...
def image_to_bchw_float(image, device):
    """
    Convert res.image (PIL / numpy / torch) to torch [1,3,H,W] float on device.
    Assumes image values are in [0,255] if uint8.
    """
    import numpy as np
    import torch
    from PIL import Image
...
def generate_drag_points(img, mask, num_points=3):
    """
    Generate drag points fully inside the editable region.

    IMPORTANT:
      - Here we define editable region as mask < 0.5 (same as your later checks).
      - Both handle_points and target_points will be inside mask < 0.5.
    """
    H, W = img.shape[:2]
    handle_points = []
    target_points = []

    # ✅ editable region: mask < 0.5
    inside_coords = np.where(mask < 0.5)
    if len(inside_coords[0]) == 0:
        print("Warning: No valid points inside editable (mask<0.5) region")
        return [], []

    rng = np.random.RandomState(42)
    min_distance = 40
    attempts = 0
    max_attempts = 2000  # increase attempts because constraint is tighter

    # helper to check distance
    def far_enough(py, px, existing, min_d):
        for (ey, ex) in existing:
            if (py - ey) ** 2 + (px - ex) ** 2 < (min_d ** 2):
                return False
        return True
    ...
    while len(handle_points) < num_points and attempts < max_attempts:
        ...
        # pick random handle point inside editable region
        idx = rng.randint(0, len(inside_coords[0]))
        hy = float(inside_coords[0][idx])
        hx = float(inside_coords[1][idx])

        # spacing constraint between handles
        if not far_enough(hy, hx, handle_points, min_distance):
            continue

        # propose target by random displacement
        dy = rng.randint(-40, 41)
        dx = rng.randint(-40, 41)

        # require meaningful move
        if not (abs(dy) > 20 or abs(dx) > 25):
            continue

        ty = np.clip(hy + dy, 0, H - 1)
        tx = np.clip(hx + dx, 0, W - 1)

        # ✅ target must also be inside editable region
        if mask[int(ty), int(tx)] < 0.5:
            handle_points.append([hy, hx])
            target_points.append([ty, tx])
            print(
                f"  Point {len(handle_points)}: handle=({hy:.1f}, {hx:.1f}), "
                f"target=({ty:.1f}, {tx:.1f}), delta=({dy}, {dx})"
            )
    ...
    # ✅ final sanity check
    for i, (h, t) in enumerate(zip(handle_points, target_points)):
        hy, hx = int(round(h[0])), int(round(h[1]))
        ty, tx = int(round(t[0])), int(round(t[1]))
        if not (mask[hy, hx] < 0.5 and mask[ty, tx] < 0.5):
            print(f"[BUG] point {i} not fully inside mask<0.5: "
                  f"mask(handle)={mask[hy,hx]:.3f}, mask(target)={mask[ty,tx]:.3f}")

    print(f"  Total generated {len(handle_points)} drag points (all inside mask<0.5)")
...
def run_drag_eval(
    ...
    print(handle_points, target_points)
    ...
    print("Step 5.0: Recording image and feature snapshots...")
    image_snapshot, snapshot_features = record_mask_snapshot(renderer, trunc_psi=0.7)
    ...
    renderer.last_image = to_raft_img(img2,renderer._device)
    
    res_drag_blend = copy.deepcopy(res_drag)
    # Run drag loop
    for step in range(drag_iterations):
        try:
            ...
            render_drag_impl(
                ...
                use_blended_output= use_cascaded_blending,
            )

            renderer.last_image = to_raft_img(res_drag.image,renderer._device)
    ...
    moved_points = res_drag.points  # moved handle points after last iteration
    print(handle_points)
    print(moved_points)
    ...
    print(moved_points, target_points)
    if use_cascaded_blending:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fixloss-type', type=str, default='single',
    ...
    run_drag_eval(
        ...
        fixloss_type=args.fixloss_type,
