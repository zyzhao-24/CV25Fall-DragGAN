...
print(f'Initializing with model: {init_pkl}')
...
with gr.Blocks() as app:
    ...
    global_state = gr.State({
        ...
        "params": {
            ...
            "trunc_cutoff": None,
            "lr": 0.001,
            "tracking_method": "mixed",  # Add tracking_method parameter
            "fixloss_type": "single",  # Add fixloss_type parameter
            "use_blended_output": False,  # Add use_blended_output parameter
        },
        "device": device,
        "draw_interval": 1,
    ...
    with gr.Row():
        ...
        with gr.Row():
            ...
            with gr.Column(scale=3):
                ...
                form_fixloss_type = gr.Dropdown(
                    choices=['single', 'blended', 'multilayer', 'raft'],
                    value=global_state.value['params']['fixloss_type'],
                    label="Fix Loss Type",
                    info="Select Fix Loss: single, blend, multilayer, and raft"
                )
                form_use_blended_output = gr.Checkbox(
                                    value=global_state.value['params']['use_blended_output'],
                                    label="Use blended output",
                                    info="Use blended image as output"
                )
                # Mask
                with gr.Row():
                    with gr.Column(scale=1, min_width=10):
    ...
    def on_change_tracking_method(tracking_method, global_state):
        """Function to handle tracking method change."""
        global_state['params']['tracking_method'] = tracking_method
        print(f'Tracking method changed to: {tracking_method}')
        return global_state

    form_tracking_method.change(
        on_change_tracking_method,
        inputs=[form_tracking_method, global_state],
        outputs=[global_state],
    )

    # Fix Loss Type change handler
    def on_change_fixloss_type(fixloss_type, global_state):
        """Function to handle fix loss type change."""
        global_state['params']['fixloss_type'] = fixloss_type
        print(f'Fix loss type changed to: {fixloss_type}')
        return global_state

    form_fixloss_type.change(
        on_change_fixloss_type,
        inputs=[form_fixloss_type, global_state],
        outputs=[global_state],
    )

    # Use blended output change handler
    def on_change_use_blended_output(use_blended_output, global_state):
        """Function to handle use blended output change."""
        global_state['params']['use_blended_output'] = use_blended_output
        print(f'[DEBUG] Use blended output changed to: {use_blended_output}')
        print(f'[DEBUG] Current editing state: {global_state.get("editing_state", "unknown")}')
        return global_state

    form_use_blended_output.change(
        on_change_use_blended_output,
        inputs=[form_use_blended_output, global_state],
    )
    ...
    def on_click_start(global_state, image):
        ...
        if len(global_state["points"]) == 0:
            ...
            yield (
                ...
                # gr.Number.update(interactive=True),
                gr.Number.update(interactive=True),
                gr.Dropdown.update(interactive=True),  # form_tracking_method
                gr.Dropdown.update(interactive=True),  # form_fixloss_type
                gr.Checkbox.update(interactive=True),  # form_use_blended_output
            )
        else:
            ...
            # Initialize last_image for RAFT
            img_pil = global_state['images']['image_raw']
            renderer.last_image = torch.from_numpy(np.array(img_pil)).permute(2, 0, 1).float().unsqueeze(0).to(device)
            ...
            while True:
                ...
                renderer._render_drag_impl(
                    ...
                    r3=global_state['params']['r3_in_pixels'],  # r3
                    # random_seed     = 0,
                    # noise_mode      = 'const',
                    ...
                    is_drag=True,
                    to_pil=True,
                    tracking_method=global_state['params']['tracking_method'],
                    fixloss_type=global_state['params']['fixloss_type'],
                    use_blended_output=global_state['params']['use_blended_output']
                    )  # Add tracking_method parameter

                # Update last_image for next step
                res_img_pil = global_state['generator_params']['image']
                renderer.last_image = torch.from_numpy(np.array(res_img_pil)).permute(2, 0, 1).float().unsqueeze(0).to(device)
                ...
                yield (
                    ...
                    # gr.Number.update(interactive=False),
                    gr.Number.update(interactive=False),
                    gr.Dropdown.update(interactive=False),  # form_tracking_method
                    gr.Dropdown.update(interactive=False),  # form_fixloss_type
                    gr.Checkbox.update(interactive=True),   # form_use_blended_output (can be toggled during running)
                )
    ...
    form_start_btn.click(
        ...
        outputs=[
            ...
            show_mask,
            form_lambda_number,
            form_tracking_method,  # Add tracking method dropdown
            form_fixloss_type,     # Add fix loss type dropdown
            # form_use_blended_output, # Add use blended output checkbox
        ],
    )
...
# gr.close_all()
print("[DBG] before queue")
app.queue(concurrency_count=3, max_size=20)
print("[DBG] before launch")
app.launch(
    ...
    show_error=True,
    debug=True,
    prevent_thread_lock=False,  # 强制不要“启动就退出”
    quiet=False                 # 强制打印信息
)
print("[DBG] after launch (you should NOT see this if it blocks)")
